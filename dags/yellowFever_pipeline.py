...
def yellowFever_pipeline():
    ...
    def extract_data(started, **context):
        ...
        try:
            ...
            sql = "SELECT stored_path FROM file_uploads WHERE primary_disease ='yellow_fever' AND file_type='disease_line_list' ORDER BY uploaded_at DESC LIMIT 1"
    ...
    def resolve_dimensions(records):
        try:
            ...
            disease = dw.get_pandas_df("SELECT disease_id FROM master_disease WHERE disease_name='yellowFever'")
            if disease is None or disease.empty:
                raise Exception("No disease found with name 'yellowFever'")
    ...
    def load_yellowFever_data(records): #check where to get the epi_year value from
        try:
            ...
            required_cols = ["disease_id", "lga_id", "state_id","case_classification","epi_week","outcome"]
            missing_cols = [col for col in required_cols if col not in df.columns]
            if missing_cols:
                raise Exception(f"Missing required columns: {missing_cols}")
            
            fact_df = df[required_cols].copy()
            fact_df.columns = ["disease_id", "lga_id", "state_id", "case_classification","epi_week","outcome"]
            ...
            (disease_id,lga_id,state_id,case_classification,epi_week,outcome)
            FROM STDIN WITH CSV
            """, buffer)

            cur.execute("""
                INSERT INTO core_surveillance_fact (
                    disease,epi_week,state,lga,suspected,confirmed,deaths
                )
                SELECT
