...
@dag(
    ...
    schedule="0 * * * *",
    catchup=False,
    tags=["line list","Yellow fever"],
    default_args={
        "on_failure_callback": log_task_failure
    }
)

def yellowFever_pipeline():
    ...
    def extract_data(started, **context):
        ...
        try:
            ...
            sql = "SELECT stored_path, id FROM file_uploads WHERE primary_disease ='yellow_fever' AND file_type='disease_line_list' ORDER BY uploaded_at DESC LIMIT 1"
            df = staging.get_pandas_df(sql)
            if df is None or df.empty:
                mark_run_skipped(started)
                raise AirflowSkipException(f"There is no upload record for Yellow Fever found in the staging database")
            else:
                    ...
                    if count > 0:
                        ...
                        if line_list_data is None or line_list_data.empty:
                            ...
                            tmp_file = tempfile.NamedTemporaryFile(
                                suffix=".parquet",
                                prefix=f"yellowfever_{started}_",
                                delete=False
                            )
    ...
    def clean_data(records):
        try:
            ...
            if "Final classification (Confirmed,Negative,Indeterminate,Pending, Not Done)" in df.columns:
                df = df.rename(columns={"Final classification (Confirmed,Negative,Indeterminate,Pending, Not Done)": "RDTResult"})

            if "DATE OF SYMPTOM ONSET" in df.columns:
                df["Year"] = pd.to_datetime(df["DATE OF SYMPTOM ONSET"], errors="coerce").dt.year.astype("Int64")
            ...
            if "result" in df.columns:
                ...
                df["case_classification"] = np.where(df["result"].isin(["confirmed", "positive"]), "confirmed","suspected")
    ...
    def resolve_dimensions(records):
        try:
            ...
            disease = dw.get_pandas_df("SELECT disease_id FROM master_disease WHERE disease_name='Yellow fever'")
            if disease is None or disease.empty:
                raise Exception("No disease found with name 'Yellow fever'")
    ...
    @task
    def load_yellowFever_data(records): #check where to get the epi_year value from
        conn = None
        cur = None
        try:
            hook = PostgresHook(postgres_conn_id=dw_conn_id)
            ...
            cur = conn.cursor()
            if not records:
                print("WARNING: load_yellowFever_data received empty list, returning empty DataFrame")
                raise ValueError("load_yellowFever_data received empty list, cannot proceed with loading data")
            ...
            raise Exception(f"Error in aggregating yellow fever surveillance data from the data warehouse: {str(e)}") from e
    ...
    loaded_data =load_yellowFever_data(resolved)
    end = end_run(run)
    cleanup = cleanup_temp(records)

    loaded_data >> end >> cleanup

yellowFever_pipeline()