...
def mpox_pipeline():
    ...
    def extract_data(started, **context):
        ...
        try:
            ...
            sql = "SELECT stored_path, id FROM file_uploads WHERE primary_disease ='mpox' AND file_type='disease_line_list' ORDER BY uploaded_at DESC LIMIT 1"
            df = staging.get_pandas_df(sql)
            if df is None or df.empty:
                mark_run_skipped(started)
                raise AirflowSkipException(f"There is no upload record for Mpox found in the staging database")
            else:
    ...
    def load_mpox_data(records):
        ...
        try:
            ...
            cur = conn.cursor()
            if not records:
                print("WARNING: load_mpox_data received empty list, returning empty DataFrame")
                raise ValueError("load_mpox_data received empty list, cannot proceed with loading data")
            ...
            fact_df.columns = ["disease_id", "onset_date", "lga_id", "state_id", "case_classification"]
            ...
            (disease_id,onset_date,lga_id,state_id,case_classification)
            FROM STDIN WITH CSV
            """, buffer)
    ...
    loaded_data =load_mpox_data(resolved)
    end = end_run(run)
