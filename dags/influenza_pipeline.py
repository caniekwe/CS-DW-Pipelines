...
def influenza_pipeline():
    ...
    def resolve_dimensions(records):
        try:
            ...
            disease = dw.get_pandas_df("SELECT disease_id FROM master_disease WHERE disease_name='Influenza'")
            if disease is None or disease.empty:
                raise Exception("No disease found with name 'Influenza'")
    ...
    @task
    def load_influenza_data(records):
        conn = None
        cur = None
        try:
            if not records:
